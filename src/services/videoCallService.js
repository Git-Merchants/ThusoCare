...
export const createVideoCall = async (userId, medicId = 2) => { // Changed default to 2
  try {
    ...
    if (medicError) {
      ...
      throw new Error('No available medic found');
    }
    ...
    const receiverId = medic.id;
    console.log('Resolved receiverId (medic id):', receiverId);
    
    const roomId = crypto.randomUUID();
    
    console.log('Inserting call with data:', {
      caller_id: userId,
    ...
    // Notify doctor (ID 2) via a dedicated channel
    const doctorChannel = supabase.channel(`doctor:${receiverId}`);
    doctorChannel.send({
      type: 'broadcast',
      event: 'new_call',
      payload: {
